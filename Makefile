...
import-sqs:
	@echo "Importing existing SQS queue..."
	terraform import aws_sqs_queue.keda_queue assignment-keda-queue || true
	terraform import aws_sqs_queue.keda_dlq assignment-keda-queue-dlq || true

# Delete existing SQS queue
delete-sqs:
	@echo "Deleting existing SQS queues..."
	@QUEUE_URL=$$(aws sqs get-queue-url --queue-name assignment-keda-queue --query QueueUrl --output text 2>/dev/null) && \
	if [ -n "$$QUEUE_URL" ]; then \
	    echo "Deleting main queue: $$QUEUE_URL"; \
	    aws sqs delete-queue --queue-url "$$QUEUE_URL"; \
	else \
	    echo "Main queue not found"; \
	fi
	@DLQ_URL=$$(aws sqs get-queue-url --queue-name assignment-keda-queue-dlq --query QueueUrl --output text 2>/dev/null) && \
	if [ -n "$$DLQ_URL" ]; then \
	    echo "Deleting DLQ: $$DLQ_URL"; \
...
delete-iam-policy:
	...
	if [ -n "$$POLICY_ARN" ]; then \
	    ...
	    VERSIONS=$$(aws iam list-policy-versions --policy-arn $$POLICY_ARN --query 'Versions[?!IsDefaultVersion].VersionId' --output text 2>/dev/null) && \
	    ...
	    echo "IAM policy not found: $$POLICY_NAME"; \
	fi

# Delete all existing resources that conflict
delete-existing-resources: delete-sqs delete-kms-alias delete-log-group delete-iam-policy
	@echo "All conflicting resources have been deleted"

# Import existing resources instead of deleting
import-kms-alias:
	@echo "Importing existing KMS alias..."
	@KMS_KEY_ID=$$(aws kms describe-key --key-id alias/eks/ha-eks --query KeyMetadata.KeyId --output text 2>/dev/null) && \
	if [ -n "$$KMS_KEY_ID" ]; then \
	    echo "Found KMS Key ID: $$KMS_KEY_ID"; \
	    terraform import 'module.eks.module.kms.aws_kms_alias.this["cluster"]' alias/eks/ha-eks; \
	else \
...
import-log-group:
	...
	terraform import 'module.eks.aws_cloudwatch_log_group.this[0]' /aws/eks/ha-eks/cluster || echo "Log group not found or already imported"
...
import-iam-policy:
	...
	if [ -n "$$POLICY_ARN" ]; then \
	    echo "Found policy ARN: $$POLICY_ARN"; \
	    terraform import aws_iam_policy.keda_sqs_policy $$POLICY_ARN; \
	else \
	    echo "IAM policy not found: $$POLICY_NAME"; \
	fi

import-existing-resources: import-sqs import-kms-alias import-log-group import-iam-policy
	@echo "All existing resources have been imported"

# Clean apply: delete conflicting resources first, then apply
clean-apply: delete-existing-resources
	@echo "Running terraform apply after cleanup..."
	make plan
	make apply-plan

# Complete fresh deployment
fresh-deploy: delete-existing-resources init
